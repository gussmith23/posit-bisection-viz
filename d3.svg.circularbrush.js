/**
 * Circular brush for selection. Downloaded from an external GitHub repo:
 * https://github.com/emeeks/d3.svg.circularbrush/tree/master
 *
 * Lightly modified to work with d3 v5 as it was originally written for d3 v3
 */

d3.svg.circularbrush = function() {
	var _extent = [0,Math.PI * 2];
    var _circularbrushDispatch = d3.dispatch('start', 'end', 'brush');
	...
	function resizeMove(_resize) {
        ...
        if (_newStartAngle - _handleSize < 0 
            || _newEndAngle + _handleSize > Math.PI * 2
    ...
    /** 
     * These two follow functions are added because they were removed from d3 after the brush 
     * implementation was originally written
     */
